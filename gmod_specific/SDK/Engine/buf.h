...
extern FORCEINLINE unsigned long LoadLittleDWord(const unsigned long *base, unsigned int dwordIndex);


extern FORCEINLINE void StoreLittleDWord(unsigned long *base, unsigned int dwordIndex, unsigned long dword);
...
{
public:
	FORCEINLINE bf_read(void) : old_bf_read()
	{
	}

	FORCEINLINE bf_read(const void *pData, int nBytes, int nBits = -1) : old_bf_read(pData, nBytes, nBits)
	{
	}

	FORCEINLINE bf_read(const char *pDebugName, const void *pData, int nBytes, int nBits = -1) : old_bf_read(pDebugName, pData, nBytes, nBits)
	{
	}
};
...
{
public:
	FORCEINLINE bf_write(void) : old_bf_write()
	{
	}
	FORCEINLINE bf_write(void *pData, int nBytes, int nMaxBits = -1) : old_bf_write(pData, nBytes, nMaxBits)
	{
	}

	FORCEINLINE bf_write(const char *pDebugName, void *pData, int nBytes, int nMaxBits = -1) : old_bf_write(pDebugName, pData, nBytes, nMaxBits)
	{
	}
};
